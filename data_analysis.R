...
data2 <- read.csv("C:/Users/HYM/Desktop/멀티캠퍼스/wordchain/kakao.csv")
data2 <- data2 %>% filter(substr(data2[,3],3,4) == "21" | substr(data2[,3],3,4) == "20")

data3 <- read.csv("C:/Users/HYM/Desktop/멀티캠퍼스/wordchain/KDBbank.csv")
...
dat <- cbind(score,count,name)
dat <- as.data.frame(dat)

par(mfrow=c(1,1))
ggplot(data = dat, aes(x = score, y = count, col=name)) + xlim(1, 5) + ylim(0, 6000) + geom_point(size=4)
